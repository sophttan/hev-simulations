...
vals <- foreach (i = 1:n_sims, .combine = 'c') %dopar% {
    ...
    if (idcs > 0.04 & idcs < 0.06) {
        write.csv(results, file = paste0('5/', i, '.csv'), row.names = F)
    }
    idcs
}
vals <- matrix(vals, n_sims, byrow = T)
saveRDS(vals, file = 'idcs_05.rds')
write.table(vals, file = 'idcs_05.txt', row.names = F, col.names = F)
...
vals <- foreach (i = 1:n_sims, .combine = 'c') %dopar% {
    results <- SEIR_p2p(params) 
    idcs <- metrics(results)
    if (idcs > 0.09 & idcs < 0.11) {
        write.csv(results, file = paste0('10/', i, '.csv'), row.names = F)
    }
    idcs
}
vals <- matrix(vals, n_sims, byrow = T)
saveRDS(vals, file = 'idcs_10.rds')
write.table(vals, file = 'idcs_10.txt', row.names = F, col.names = F)
...
vals <- matrix(vals, n_sims, byrow = T)
saveRDS(vals, file = 'idcs_30.rds')
write.table(vals, file = 'idcs_30.txt', row.names = F, col.names = F)