...
SEIR_blend <- function(params, inf, verbose = F) {
  ...
  data <- data.frame(ID = 1:N,
                     ...
                     I_count = 0, 
                     R = 0, 
                     INC = c(round(rlnorm(1, meanlog = log(29.8), 
                                          sdlog = 0.45)), 
                             rep(0, N - 1)),
                     INF = 0)
  ...
  for(t in 1:time) {
    ...
    if (num_new_exposed > 0) {
      ...
      random_inc <- rlnorm(num_new_exposed, meanlog = log(29.8), 
                           sdlog = 0.45) %>% round()
...
beta_Hs <- seq(10, 45, 1) # 36
beta_Cs <- seq(0, 0.050, 0.005) # 11
beta_Es <- seq(0.00005, 0.00100, 0.00005) # 20
...
idcs <- readRDS('idcs.rds')
sars <- readRDS('sars.rds')
prps <- readRDS('prps.rds')
...
for (i in 29:d_H) {
  for (j in 1:d_C) {
    for (k in 1:d_E) {
      ...
      cat(paste0(format(beta_H, nsmall = 0), '/45\t', 
                 format(beta_C, nsmall = 3), '/0.050\t',
                 format(beta_E, nsmall = 5, scientific = F), '/0.00100\t'))
        
      t_0 <- Sys.time()
      vals <- foreach (l = 1:reps, .combine = 'c') %dopar% {
        results <- SEIR_blend(params, inf) 
        metrics(results)
      }
      t_1 <- Sys.time()
      t_tot <- t_tot + (t_1 - t_0)
