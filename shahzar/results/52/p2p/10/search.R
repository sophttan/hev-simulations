...
beta_Cs <- seq(0.09, 0.11, 0.001) #21
...
for (i in 1:d_H) {
    for (j in 1:d_C) {
      ...
      cat(paste0(format(beta_H, nsmall = 1, digits = 3), '/42.0\t', 
                 format(beta_C, nsmall = 3, digits = 3), '/0.110\t'))
      ...
      cat(paste0(format(round(mean(idcs[i, j, ]), 3), nsmall = 3), '\t',
                 format(round(mean(idcs[i, j, idcs[i, j, ] > 0.09 & idcs[i, j, ] < 0.11]), 3), nsmall = 3), '\t',
                 format(round(mean(sars[i, j, ]), 3), nsmall = 3), '\t', 
                 format(round(mean(sars[i, j, idcs[i, j, ] > 0.09 & idcs[i, j, ] < 0.11]), 3), nsmall = 3), '\n'))
