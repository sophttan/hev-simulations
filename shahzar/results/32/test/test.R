...
beta_Hs <- seq(30, 70, 10)
beta_Cs <- seq(0, 1, 0.25)
beta_Es <- seq(0, 0.02, 0.005)
  ...
  for (j in 1:b) {
    for (k in 1:c) {
      ...
      cat(paste0(beta_H, '/70\t', 
                 ...
                 format(beta_E, nsmall = 3, scientific = F), '/0.020\t'))
      ...
      cat(paste0(format(t_tot, digits = 4, nsmall = 2), '\t(', 
                 format(t_1 - t_0, digits = 4, nsmall = 2), ')\t'))
