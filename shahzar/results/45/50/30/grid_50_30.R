...
beta_Hs <- seq(27.5, 30.5, 0.2) # 16
beta_Cs <- seq(0.005, 0.015, 0.001) # 11
beta_Es <- seq(0.00045, 0.00065, 0.00001) # 21
...
for (i in 1:d_H) {
  for (j in 1:d_C) {
    for (k in 1:d_E) {
      ...
      cat(paste0(format(beta_H, nsmall = 0), '/30.5\t', 
                 format(beta_C, nsmall = 3), '/0.015\t',
                 format(beta_E, nsmall = 5, scientific = F), '/0.00065\t'))
