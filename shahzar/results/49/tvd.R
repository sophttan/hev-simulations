...
tvd <- function(samples_1, samples_2, bins) {
  freq_1 <- hist(samples_1, breaks = bins, include.lowest = TRUE, plot = FALSE)
  freq_2 <- hist(samples_2, breaks = bins, include.lowest = TRUE, plot = FALSE)
  
  freq_1 <- freq_1 / length(samples_1)
  freq_2 <- freq_2 / length(samples_2)
  
  return(0.5 * sum(abs(freq_1 - freq_2)))
