...
SEIR_environment <- function(b, inc, inf) {
  ...
  for (i in 1:time) {
    ...
    if(sum(new_exposed)>0) {
      new_exposed <- rep(new_exposed, times=hh_size)
      d$E[new_exposed==1] <- 1
...
write_csv(inf_type, "simulated_data/environmental_fixedhhrisk.csv")
...
p %>% ggsave(filename = "figures/pred_environmental_fixedhhrisk.jpg")
