...
env3 <- read_csv("simulated_data/environmental_fixedhhrisk_lag.csv")
...
env3_summ <- env3 %>% prep_results()

inf_type <- env_summ %>% 
  full_join(env2_summ, by="month") %>% 
  full_join(env3_summ, by="month")
...
p <- inf_type %>% ggplot(aes(month)) + 
  ...
  geom_line(aes(y=has_hh, color="Environmental transmission\nto households with\nvariable lag")) + 
