...
SEIR <- function(beta_H, beta_C, inc, inf, verbose = 0) {
    ...
    for(t in 1:time) {
        ...
        if (sum(new_exposed) > 0) {
            ...
            # Get people with the smallest I_counts.
            dd = data %>% filter(data$I == 1) %>% group_by(HH) %>% mutate(MIN = min(I_count))
            min_IDs = dd[dd$I_count == dd$MIN, ]$ID
            ...
            curr_I_num = results[data$ID %in% min_IDs & data$HH %in% new_HHs, ]$I_num
            if (length(curr_I_num) != length(rr$I_tot)) {
              return(list(data, results, new_inf_H))
            }
            results[results$ID %in% min_IDs & results$HH %in% new_HHs, ]$I_num = curr_I_num + rr$I_tot
...
metrics = function(results) {
    ...
    if (idc != 0) {
        ...
        num_primary = sum(results %>% group_by(HH) %>% summarise(time_sum = sum(time, na.rm = T)) %>% ungroup() %>% select(time_sum) > 0)
        idx = results %>% select(type) %>% sapply(f)
        num_contact = results %>% group_by(HH) %>%
