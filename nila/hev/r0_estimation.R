...
# data <- read_csv(here("results/separate_models/cumulative_inc_5/simulated_data/environmental.csv"))
# 
# data
...
# turned the for the loop into a function!
res_ben = function(data) {
  ...
  res<-NULL
  for (j in unique(t$i)) {
    # for each simulation in 1000 (or sims many) simulations, estimate total R and total R from households
    data <- t %>% filter(i==j) %>% mutate(id=1:n())
    R<-rep(0, nrow(data))
    R_hh<-rep(0, nrow(data))
    for (k in 1:nrow(data)) {
  ...
  res_summary = (res %>% group_by(i) %>% summarise(R=mean(R), R_hh=mean(R_hh), R_hh/R)) %>% summarise_all(mean)
  return(res_summary)
}
