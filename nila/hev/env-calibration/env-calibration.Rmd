...
Notice that when calculating $I_c$ we don't particularly care about which people leave the infected group, only who enters. We can then interpret $I_c$ as in integral of just this term.
...
```{r calibration}
g = function(beta){
  cum_inc = -N*exp(-beta*(t - e_t)/N) + N*exp(e_t*beta/N) - N*p
  abs(cum_inc)
}
